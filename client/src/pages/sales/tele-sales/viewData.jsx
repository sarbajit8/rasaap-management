...
import { DrumstickIcon, Pen, PenBox, PenIcon, Save, Trash, Trash2Icon } from "lucide-react";
...
const ViewData = () => {
  ...
  // Ensure that companyDetails and its companies are properly fetched before attempting to access them
  const sortedCompanyDetails = companyDetails
    ?.flatMap((detail) => detail?.companies || [])
    .filter((company) => company.assigned === employeeName) // Filter by employee name
    ...
    <div key={index} className="space-y-4 border p-4 rounded-md shadow-sm">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <input
          ...
          className="border rounded p-2"
        />
        {/* Set 'assigned' to employeeName as the default value */}
        <input
          type="text"
          name="assigned"
          value={employeeName} // Set default to employeeName
          readOnly
          className="border rounded p-2"
        />
        <select
        ...
        <tbody>
          {sortedCompanyDetails?.length === 0 ? (
            ...
            sortedCompanyDetails
              .filter((company) => company.assigned === employeeName)
              .filter(
                (company) =>
